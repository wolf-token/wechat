<?php 
	...
	if ($operation == "list") {
		...
		if (!empty($staff)) {
			...
			//查找未确认的订单
			$order = pdo_fetchall("SELECT * FROM ".tablename("move_declare")." WHERE pid=:pid AND status=4 AND appraise=0 ",array("pid"=>$staff['id']));
		...
		if (!empty($_GPC['id'])) {
			...
			$result = pdo_update("move_declare",array("status"=>1),array("id"=>$id));
			...
			if (!empty($result)) {
				...
				message("确认订单信息失败",$this->createMobileUrl("order",array("op"=>'list')),"info");
			}
