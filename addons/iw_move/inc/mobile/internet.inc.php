<?php 
	...
	if ($operation == "add") {
		...
		// }else if(!empty($person) && !empty($person['name']) && !empty($person['openid'])){
			...
			//查找固话信息
			$human = pdo_fetch("SELECT * FROM ".tablename("move_internet")." WHERE id=:id",array("id"=>$_GPC['id']));

			//加载信息
			include $this->template("human",$person);
		...
		//查找信息
		$internet = pdo_fetchall("SELECT * FROM ".tablename("move_internet")." WHERE status=0 ");

		//判断是否存在信息
		if (!empty($internet)) {
			
			//加载页面
			include $this->template("internet");
		...
		//查找信息
		$internet_details = pdo_fetch("SELECT * FROM ".tablename("move_internet")." WHERE status=0 AND id=:id",array("id"=>$_GPC['id']));
		$internet_details['content'] = htmlspecialchars_decode($internet_details['content']);
		// var_dump($internet_details['content']);die();
		//判断是否存在信息
		if (!empty($internet_details)) {
			
			//加载页面
			include $this->template("internet_details");
		...
		if ($_W['ispost']) {
			...
			//查找固话信息
			$tell = pdo_fetch("SELECT * FROM ".tablename("move_internet")." WHERE status=0 AND id=:id",array("id"=>$_GPC['id']));
			//模糊查找
			$where = '';
