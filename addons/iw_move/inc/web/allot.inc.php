<?php 
	...
	if ($operation == "list") {
				    ...
				    if (!empty($message)) {
				        ...
				        if ($res[errcode]==0){
				           ...
				           header('Location: '.$this->createWebUrl("fault",array("op"=>"list")));
				            ...
				            message("分配员工失败,等待管理员分配",$this->createWebUrl("allot",array("op"=>"list")),"error");
				        }
				        ...
				        message("未找到与之匹配的维修工,等待管理员分配",$this->createWebUrl("allot",array("op"=>"list")),"error");
				    }
		...
		//查取信息
		$allot_edit = pdo_fetch("SELECT * FROM ".tablename("move_declare")." WHERE id=:id",array("id"=>$_GPC['id']));
		
		
		//加载页面
		include $this->template("allot_edit");
