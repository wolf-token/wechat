<?php 
	...
	if ($operation == "add") {
        ...
        foreach($values as $v){
            ...
            $valueStr .= "('".$v[2]."','".$v[3]."','".$v[1]."','".$v[0]."','".$time."','".$pass."'),";
        } 

        $valueStr = rtrim($valueStr,',');
        $sql = "INSERT IGNORE INTO `ims_move_staff`(name,phone,team,tid,time,pass) values $valueStr";
        ...
        if (!empty($result1)) {
            ...
            message("导入员工信息成功",$this->createWebUrl("staff",array("op"=>"list")));
        }else{

            message("数据重复,导入员工信息失败",$this->createWebUrl("staff",array("op"=>"file")));
