...
else if ($operation == 'post') 
{
	...
	{
		$data = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'thumb' => trim($_GPC['thumb']), 'contact' => trim($_GPC['contact']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'member_id' => trim($_GPC['member_id']), 'deduct_credit1' => trim($_GPC['deduct_credit1']), 'deduct_credit2' => trim($_GPC['deduct_credit2']), 'settle_platform' => trim($_GPC['settle_platform']), 'settle_store' => trim($_GPC['settle_store']), 'commission1_rate' => trim($_GPC['commission1_rate']), 'commission2_rate' => trim($_GPC['commission2_rate']), 'commission3_rate' => trim($_GPC['commission3_rate']), 'credit1' => trim($_GPC['credit1']), 'creditpack' => trim($_GPC['creditpack']), 'redpack_min' => trim($_GPC['redpack_min']), 'redpack' => trim($_GPC['redpack']), 'deredpack' => trim($_GPC['deredpack']), 'decommission' => trim($_GPC['decommission']), 'decredits' => trim($_GPC['decredits']), 'iscontact' => trim($_GPC['iscontact']));
		if ($pcoupon) 
		{
		...
		if (!empty($id)) 
		{
			...
			{
				$_var_156 = array( 'keyword1' => array('value' => '收银台商户通知', 'color' => '#73a68d'), 'keyword2' => array('value' => '【商户名称】' . $cashier_stores['name'], 'color' => '#73a68d'), 'remark' => array('value' => '您于此商户绑定的微信角色已经被替换!') );
				$_var_157 = array( 'keyword1' => array('value' => '收银台商户通知', 'color' => '#73a68d'), 'keyword2' => array('value' => '【商户名称】' . $cashier_stores['name'], 'color' => '#73a68d'), 'remark' => array('value' => '此商户绑定的微信角色已经成功更换成您!') );
		...
		else 
		{
			...
			$_var_157 = array( 'keyword1' => array('value' => '收银台商户通知', 'color' => '#73a68d'), 'keyword2' => array('value' => '【商户名称】' . $cashier_stores['name'], 'color' => '#73a68d'), 'remark' => array('value' => '您已经被绑定成为此商户的微信角色!') );
			$id = pdo_insertid();
...
else if ($operation == 'delete_waiter') 
{
	ca('cashier.store.delete');
	$id = intval($_GPC['id']);
	$sid = intval($_GPC['sid']);
