...
if ($_W['isajax']) 
{
	if ($operation == 'display') 
	{
		...
		foreach ($goods as &$g ) 
		{
			...
			if (is_array($discounts)) 
			{
				if (!empty($level['id'])) 
				{
					...
					else if ((0 < floatval($level['discount'])) && (floatval($level['discount']) < 10)) 
					{
				...
				else if ((0 < floatval($discounts['default'])) && (floatval($discounts['default']) < 10)) 
				{
		...
		show_json(1, array('member' => $member, 'deductcredit' => $deductcredit, 'deductmoney' => $deductmoney, 'deductcredit2' => $deductcredit2, 'saleset' => $saleset, 'goods' => $goods, 'weight' => $weight / $buytotal, 'set' => m('common')->getSysset('shop'), 'fromcart' => $fromcart, 'haslevel' => !empty($level['id']) && (0 < $level['discount']) && ($level['discount'] < 10), 'total' => $total, 'dispatchprice' => number_format($dispatch_price, 2), 'totalprice' => number_format($totalprice, 2), 'goodsprice' => number_format($goodsprice, 2), 'discountprice' => number_format($discountprice, 2), 'discount' => $level['discount'], 'realprice' => number_format($realprice, 2), 'address' => $address, 'carrier' => $stores[0], 'carrier_list' => $stores, 'dispatch_list' => $dispatch_list, 'isverify' => $isverify, 'isverifysend' => $isverifysend, 'stores' => $stores, 'isvirtual' => $isvirtual, 'changenum' => $changenum, 'hascoupon' => $hascoupon, 'couponcount' => $couponcount));
	}
		...
		if (!empty($goods)) 
		{
			...
			foreach ($goodsarr as &$g ) 
			{
				...
				if (is_array($discounts)) 
				{
					...
					else if ((0 < $discounts['default']) && ($discounts['default'] < 10)) 
					{
	...
	else if (($operation == 'create') && $_W['ispost']) 
	{
		...
		foreach ($goodsarr as $g ) 
		{
			...
			else if (!empty($diyformtype) && !empty($data['diyformid'])) 
			{
		...
		if ($saleset) 
		{
			if (!empty($saleset['enoughfree'])) 
			{
				...
				else if (floatval($saleset['enoughorder']) <= $totalprice) 
				{
					if (empty($saleset['enoughareas'])) 
					{
						$dispatch_price = 0;
		...
		if (p('supplier')) 
			{
				$order['supplier_uid'] = $data['supplier_uid'];
			}
