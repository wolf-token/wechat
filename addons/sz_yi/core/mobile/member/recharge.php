...
if (($operation == 'display') && $_W['isajax']) 
{
	...
	show_json(1, array('set' => $set, 'logid' => $logid, 'isweixin' => is_weixin(), 'wechat' => $wechat, 'alipay' => $alipay, 'app_wechat' => $app_wechat, 'app_alipay' => $app_alipay, 'credit' => $credit, 'yunpay' => $yunpay, 'payno' => $logno));
}
...
{
	$logid = intval($_GPC['logid']);
	$log = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_member_log') . ' WHERE `id`=:id and `openid`=:openid and `uniacid`=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $logid));
