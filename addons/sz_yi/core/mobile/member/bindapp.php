	...
	if ($_W['ispost']) 
	{
		if (is_weixin()) 
		{
			if (empty($member)) 
			{
				...
				$exists = pdo_fetchcolumn('SELECT `id` FROM ' . tablename('sz_yi_member') . ' WHERE mobile=:mobile AND uniacid=:uniacid', array('mobile' => $_GPC['mobile'], 'uniacid' => $_W['uniacid']));
				if ($exists) 
				{
					pdo_update('sz_yi_member', array('uid' => $member['uid'], 'openid' => $member['openid'], 'realname' => $member['realname'], 'nickname' => $member['nickname'], 'avatar' => $member['avatar'], 'gender' => $member['gender'], 'province' => $member['province'], 'city' => $member['city'], 'area' => $member['area'], 'createtime' => $member['createtime'], 'bindapp' => 1), array('mobile' => $_GPC['mobile'], 'uniacid' => $_W['uniacid']));
					show_json(1, array('redireurl' => $redireurl));
				}
				else 
				{
					show_json(-1);
